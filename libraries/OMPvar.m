function [S]=OMPvar(Phi,V,m,alpha,errorGoal)
%==========================================================================
...
for j=1:1:P,
    ...
    k=0;
    v = []; b = []; Ai = [];
    for l = 1:length(maxNumCoef)
        while currResNorm2>errorGoal && k < maxNumCoef(l),
            k = k+1;
            %-- Atom Selection---
            proj= Phi'*residual;
            [~,pos]= max(abs(proj));
